#include "BlackCmdListRecorder.h"
...
BlackCmdListRecorder::BlackCmdListRecorder(ID3D12Device& device, tbb::concurrent_queue<ID3D12CommandList*>& cmdListQueue)
	: CmdListRecorder(device, cmdListQueue)
{
}

void BlackCmdListRecorder::RecordCommandLists(
	...
	ASSERT(geomPassRtvCpuDescHandlesCount > 0U);
	...
	mCmdList->OMSetRenderTargets(geomPassRtvCpuDescHandlesCount, geomPassRtvCpuDescHandles, true, &depthStencilHandle);
