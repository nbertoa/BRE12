...
namespace {
	...
	void GenerateRecorder(
		...
		recorder = new ColorHeightCmdListRecorder();
...
void ColorHeightScene::Init() noexcept {
	Scene::Init();
...
void ColorHeightScene::CreateGeometryPassRecorders(
	...
	GenerateRecorder(sTx1, sTy1, sTz1, sOffsetX1, 0.0f, 0.0f, sS, model.GetMeshes(), normal, height, materials, numResources, recorder);
	ASSERT(recorder != nullptr);
	tasks.push_back(std::unique_ptr<GeometryPassCmdListRecorder>(recorder));
}

void ColorHeightScene::CreateLightingPassRecorders(
	Microsoft::WRL::ComPtr<ID3D12Resource>* geometryBuffers,
	const std::uint32_t geometryBuffersCount,
	ID3D12Resource& depthBuffer,
