...
void HeightCmdListRecorder::InitPSO(const DXGI_FORMAT* geometryBufferFormats, const std::uint32_t geometryBufferCount) noexcept {
	...
	ShaderManager::LoadShaderFile("GeometryPass/Shaders/HeightMapping/DS.cso", psoData.mDomainShaderBytecode);
	ShaderManager::LoadShaderFile("GeometryPass/Shaders/HeightMapping/HS.cso", psoData.mHullShaderBytecode);
	ShaderManager::LoadShaderFile("GeometryPass/Shaders/HeightMapping/PS.cso", psoData.mPixelShaderBytecode);
	ShaderManager::LoadShaderFile("GeometryPass/Shaders/HeightMapping/VS.cso", psoData.mVertexShaderBytecode);

	ID3DBlob* rootSignatureBlob;
	ShaderManager::LoadShaderFile("GeometryPass/Shaders/HeightMapping/RS.cso", rootSignatureBlob);
...
void HeightCmdListRecorder::RecordAndPushCommandLists(const FrameCBuffer& frameCBuffer) noexcept {
	...
	mCommandList->SetGraphicsRootConstantBufferView(5U, frameCBufferGpuVAddress);
	...
	for (std::size_t i = 0UL; i < geomCount; ++i) {
		...
		for (std::size_t j = 0UL; j < worldMatsCount; ++j) {
			...
			mCommandList->SetGraphicsRootDescriptorTable(3U, heightsBufferGpuDesc);
			heightsBufferGpuDesc.ptr += descHandleIncSize;

			mCommandList->SetGraphicsRootDescriptorTable(4U, materialsCBufferGpuDesc);
			materialsCBufferGpuDesc.ptr += descHandleIncSize;

			mCommandList->SetGraphicsRootDescriptorTable(6U, texturesBufferGpuDesc);
			texturesBufferGpuDesc.ptr += descHandleIncSize;

			mCommandList->SetGraphicsRootDescriptorTable(7U, normalsBufferGpuDesc);
